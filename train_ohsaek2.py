...
def objective(trial):
    ...
    #         'reg_lambda': trial.suggest_float('reg_lambda', 0.4, 1.0),
    #         'random_state': trial.suggest_int('random_state', 1, 50),
    #     }
    params = {
        'max_depth': trial.suggest_int('max_depth', 5, 20),
        'learning_rate': trial.suggest_float('learning_rate', 0.5, 0.9),
        'min_child_weight': trial.suggest_int('min_child_weight', 5, 8),
        'gamma': trial.suggest_float('gamma', 0.1, 1.0),
        'subsample': trial.suggest_float('subsample', 0.3, 1.0),
        'colsample_bytree': trial.suggest_float('colsample_bytree', 0.1, 1),
        'reg_alpha': trial.suggest_float('reg_alpha', 0.1, 0.1),
        'reg_lambda': trial.suggest_float('reg_lambda', 0.1, 0.1),
    ...
    model = xgb.train(params,
                      ...
                      verbose_eval = False)           
    pred = model.predict(dval)
    wmae = metric.weighted_mae_metric_ohsaek2(pred, dval)
    return wmae[1]
